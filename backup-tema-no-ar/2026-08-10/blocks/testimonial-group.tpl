...
<div class="testimonial-group text-{{ alignment }}" {{ block | block_attributes }} data-store="testimonial-group-{{ block.id }}">
	{% if has_items %}
		{% if use_slider %}
			<div class="js-testimonials-slider-container testimonials-slider-wrapper" data-columns-desktop="{{ columns }}">
		...
		{% if use_slider %}
				...
				{% if item_count > 1 %}
					...
					<div class="js-swiper-testimonials-pagination swiper-pagination swiper-pagination-outside"></div>
				{% endif %}
			</div>
		{% else %}
			</div>
